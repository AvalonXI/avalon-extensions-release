...
charm_data = require_generated_module('data/charm_data.lua', 'charm_data.lua', 'charm_data');
...
-- Keep panel colors out of settings because legacy integers collide with RGBA tables.
local PANEL_COLORS = {
...
local function load_charm_lookup()
    ...
    charm_lookup = require_generated_module('data/charm_lookup.lua', 'charm_lookup.lua', 'charm_lookup');
    return charm_lookup;
end
...
ashita.events.register('packet_in', 'charm_chance_packet_in_cb', function (e)
    -- Zone packet 0x00A invalidates the cached merit table.
    if (e.id == 0x00A) then
        merit_cache = {};
        return;
    end

    -- Packet 0x08C provides the merit entries parsed below.
    if (e.id ~= 0x08C) then
        return;
    end
...
-- ImGui flips this ref to false when the title-bar [X] is clicked.
...
local function maybe_save_layout()
    ...
    -- The box auto-sizes; persist position only.
    if (hud.window_pos_x ~= pos_x or hud.window_pos_y ~= pos_y) then
