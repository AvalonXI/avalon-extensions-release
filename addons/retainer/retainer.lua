...
-- Sync collects dump lines until the header's N lands or a timeout fires; the cache is replaced only when the full expected count arrives or the retainer is explicitly empty.
...
local sync = {
    ...
    collected      = 0,
    empty_confirmed = false,      -- server explicitly reported an empty retainer
    idle_deadline  = 0,
    hard_deadline  = 0,
};
local last_sync_request = -math.huge;
-- Arm syncs per character so leftover dump lines cannot commit into a new cache.
...
local function commit_sync()
    -- Keep the previous cache unless the dump is complete or explicitly empty.
    local complete = (sync.expected ~= nil and sync.collected >= sync.expected);
    if (complete or sync.empty_confirmed) then
        local fresh = T{};
        ...
        set_status('Sync incomplete -- previous list kept. Click Sync to retry.');
    end
    sync.active         = false;
    sync.buffer         = {};
    sync.empty_confirmed = false;
...
local function withdraw(e, amount)
    ...
    -- Reconcile this optimistic decrement if the server later refuses it.
...
local function handle_incoming(original)
    ...
    if (hn) then
        ...
        if (not sync.active) then
            if (not sync_armed) then
                return true;  -- unsolicited/leftover header; ignore until this char requests a sync
            end
            begin_sync();
        end
    ...
    if (qty) then
        ...
        if (id) then
            if (not sync.active) then
                if (not sync_armed) then
                    return true;  -- stray dump line (e.g. previous character's tail); ignore
                end
                begin_sync();
            end
